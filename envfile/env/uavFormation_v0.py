...
class uavFormationEnv0(uavFormationbase):
    ...
    def reset(self, **kwargs):
        ...
        for jj in range(self.nb_agents):
            ...
            obs_dict[self.agents[jj].agent_id].append([1, r, alpha, 0])
    ...
    def step(self, action_dict):
        ...
        for ii, agent_id in enumerate(action_dict):
            ...
            obs_dict[agent_id].append([1 if self.done_dict[self.agents[ii].agent_id] is True
                                       else 0, r, alpha, action_vw[1]])    # 状态观测更新
            obs_dict[agent_id] = np.asarray(obs_dict[agent_id])
        ...
        if self.episode_record == self.destroy_happened:
            destroy_start_number = int(np.random.randint(1, self.nb_agents, 1))
            destroy_end_number = int(np.min([destroy_start_number + self.destroy_number, self.nb_agents]))  # 留下来的第一个编号
            for gg in range(destroy_end_number - destroy_start_number):
                del obs_dict['agent-' + str(destroy_start_number - 1 + gg)]
            del self.agents[destroy_start_number - 1: destroy_end_number - 1]
            del self.targets[destroy_start_number - 1: destroy_end_number - 1]
            really_destroy_number = np.min([self.nb_agents - destroy_start_number + 1, self.destroy_number])
        ...
        if self.episode_record == self.destroy_happened + 5:
            init_pose_target = shape_regeneration(pre_number=self.pre_number_targets, new_number=self.nb_targets,
                                                  ...
                                                  formation_name=METADATA['shape_name'])
            for nn in range(self.nb_targets):
                self.targets[nn].reset(np.array(init_pose_target[nn]) + np.array([x_change, y_change, 0]))
